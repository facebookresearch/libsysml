...
#    include "sysml/code_generator/x86/oprof-jitdump.hpp"
...
{
    template <class T>
    null_logger& operator<<([[maybe_unused]] T const& v)
    {
#    ifndef NDEBUG
        std::cerr << v;
#    endif
...
#    define SYSML_CODEGEN_PERF_LOG ::sysml::code_generator::detail::null_logger
...
#    endif
    ...
    void initPerfJitDump()
    {
        ...
        SYSML_CODEGEN_PERF_LOG()
            << "Jitdump file: " << m_perfJitDumpName << "\n";
        ...
        if (!perfGetTimestamp())
        {
            ...
            {
                SYSML_CODEGEN_PERF_LOG()
                    << "system arch timestamp not supported\n";
            }
            else
            {
                SYSML_CODEGEN_PERF_LOG()
                    << "kernel does not support (monotonic) " << perf_clk_id
                    << " clk_id\n";
            }
            SYSML_CODEGEN_PERF_LOG()
                << "Cannot create " << m_perfJitDumpName << "\n";
            return;
        }
        ...
        {
            SYSML_CODEGEN_PERF_LOG()
                << "Failed to create the file " << m_perfJitDumpName
        ...
        {
            SYSML_CODEGEN_PERF_LOG()
                << "Failed to create mmap marker file for perf"
                << "\n";
            close(fd);
            return;
        }
        ...
        {
            SYSML_CODEGEN_PERF_LOG() << "failed to get machine ELF information"
                                     << "\n";
...
inline detail::profiler_wrapper& get_x86_profiler()
{
